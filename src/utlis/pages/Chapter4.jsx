import React from "react";

const Chapter4 = () => {
  return (
    <div className="flex flex-col items-center">
      <div className="bg-orange-500 h-16 w-full flex items-center justify-center">
        <h1 className="font-samarkan text-5xl text-black">Chanakya Neethi</h1>
      </div>

      <h1 className="font-samarkan text-5xl mt-12">Chapter 4</h1>

      <div className="relative flex flex-col items-center mt-10 m-5 max-w-screen-lg mx-auto">
        {/* Verse 1 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            आयुः कर्म च वित्तं च विद्या निधनमेव च । <br />
            पञ्चैतानि हि सृज्यन्ते गर्भस्थस्यैव देहिनः ॥ 01 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              जीव का जीवनकाल (आयु), उसके कर्म (भाग्य), धन, शिक्षा, और मृत्यु ये
              पाँचों चीजें गर्भ में ही तय हो जाती हैं।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> जैसे किसी व्यक्ति का जन्म किसी अमीर या गरीब
              परिवार में होना पहले से ही तय होता है, वैसे ही उसका जीवनकाल, धन,
              शिक्षा, और कर्म भी नियति द्वारा निर्धारित होते हैं।
              <br />
              <br />
              <b>English:</b> For instance, a person's birth in a wealthy or
              poor family is predetermined, just as their lifespan, fortune,
              education, and fate are considered to be predestined.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              यह श्लोक नियति और भाग्य की अवधारणा को स्वीकार करता है, जो कहता है
              कि कुछ चीजें हमारे नियंत्रण में नहीं हैं और जीवन में पहले से ही
              निर्धारित होती हैं।
            </p>
          </div>
        </div>

        {/* Verse 2 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            साधुभ्यस्ते निवर्तन्ते पुत्रमित्राणि बान्धवाः । <br />
            ये च तैः सह गन्तारस्तद्धर्मात्सुकृतं कुलम् ॥ 02 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              जो सज्जन पुरुष होते हैं, उनके पुत्र, मित्र, और संबंधी भी अच्छे
              होते हैं। जो लोग सज्जनों का साथ देते हैं, उनके परिवार का विकास
              धर्म और अच्छे कर्मों से होता है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> जैसे यदि परिवार का मुखिया एक ईमानदार और
              धर्मपरायण व्यक्ति है, तो उसके पुत्र, मित्र, और अन्य संबंधी भी उसकी
              अच्छाइयों का अनुसरण करते हैं और एक अच्छे परिवार का निर्माण करते
              हैं।
              <br />
              <br />
              <b>English:</b> For instance, if the head of a family is a
              virtuous and honest person, his children, friends, and other
              relatives are likely to follow in his footsteps, creating a family
              known for its integrity and values.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              सज्जनों का संग उनके परिवार और मित्रों के चरित्र को भी सकारात्मक
              रूप से प्रभावित करता है। अच्छे गुणों का असर उनके पूरे कुल पर पड़ता
              है, जिससे समाज में उनका परिवार सुकृतियों से जाना जाता है।
            </p>
          </div>
        </div>

        {/* Verse 3 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            दर्शनध्यानसंस्पर्शैर्मत्सी कूर्मी च पक्षिणी । <br />
            शिशुं पालयते नित्यं तथा सज्जन-सङ्गतिः ॥ 03 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              मछली अपने बच्चे को केवल देखकर पालती है, कछुआ ध्यान से पालता है, और
              पक्षी उसे स्पर्श करके पालता है। इसी प्रकार सज्जनों की संगति भी
              हमारे चरित्र का निर्माण करती है और हमें जीवन में उत्तम मार्ग पर
              बनाए रखती है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> जैसे कोई बच्चा यदि अच्छे विचारों वाले और
              प्रेरणादायक व्यक्तियों के संपर्क में रहता है, तो उस पर स्वतः अच्छे
              संस्कारों का प्रभाव पड़ता है और वह जीवन में सफल बनता है।
              <br />
              <br />
              <b>English:</b> For instance, if a child is around inspiring and
              virtuous individuals, the child naturally absorbs positive values
              and grows to lead a successful and principled life.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              सज्जनों का संग व्यक्ति के चरित्र का निर्माण करता है और उसे
              सन्मार्ग पर ले जाता है, ठीक वैसे ही जैसे माता-पिता अपने बच्चों की
              सुरक्षा और पालन में अलग-अलग तरीके अपनाते हैं।
            </p>
          </div>
        </div>

        {/* Verse 4 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            यावत्स्वस्थो ह्ययं देहो यावन्मृत्युश्च दूरतः । <br />
            तावदात्महितं कुर्यात्प्राणान्ते किं करिष्यति ॥ 04 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              जब तक यह शरीर स्वस्थ है और मृत्यु दूर है, तब तक अपने आत्मकल्याण के
              लिए प्रयत्न करना चाहिए। प्राणों के अंत में कोई कुछ नहीं कर सकता
              है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> जैसे एक विद्यार्थी को परीक्षा के समय ही अध्ययन
              करना चाहिए, बाद में परिणाम आने के बाद पढ़ने का कोई लाभ नहीं। इसी
              प्रकार, हमें जीवन रहते हुए अच्छे कार्यों में संलग्न होना चाहिए,
              क्योंकि मृत्यु के बाद कुछ भी करने का अवसर नहीं रहता।
              <br />
              <br />
              <b>English:</b> For example, a student should study before exams
              when they have time, as studying afterward serves no purpose.
              Similarly, one should engage in virtuous deeds during life, as
              after death, there will be no opportunity left.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              इस श्लोक का सार यह है कि हमें जीवन में स्वस्थ रहते हुए अपने कल्याण
              के कार्यों में संलग्न होना चाहिए। मृत्यु के बाद या प्राणांत के
              निकट कोई भी प्रयास फलदायक नहीं होता।
            </p>
          </div>
        </div>

        {/* Verse 5 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            कामधेनुगुणा विद्या ह्यकाले फलदायिनी । <br />
            प्रवासे मातृसदृशी विद्या गुप्तं धनं स्मृतम् ॥ 05 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              विद्या (ज्ञान) कामधेनु के समान है, जो किसी भी समय उपयोगी और फलदायी
              होती है। परदेश में यह माता के समान सहायक होती है और इसे गुप्त धन
              की तरह माना जाता है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> जैसे एक व्यापारी को उसके पैसे से मदद मिल सकती
              है, एक विद्वान को अपने ज्ञान से हर जगह सम्मान और सहायता प्राप्त
              होती है। अगर वह किसी नई जगह पर जाए तो उसका ज्ञान उसकी सबसे बड़ी
              ताकत और सहारा बनता है।
              <br />
              <br />
              <b>English:</b> For instance, while a businessman relies on money,
              a scholar relies on knowledge, which earns them respect and
              assistance everywhere. If they travel to an unfamiliar place,
              their knowledge serves as their greatest asset and support.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              विद्या एक अमूल्य संपत्ति है जो हर समय और हर परिस्थिति में फल देती
              है। यह जीवन की हर कठिनाई में सहायता करने वाली "कामधेनु" के समान है
              और इसे गुप्त धन के रूप में हमेशा सहेज कर रखना चाहिए।
            </p>
          </div>
        </div>

        {/* Verse 6 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            एकोऽपि गुणवान्पुत्रो निर्गुणेन शतेन किम् । <br />
            एकश्चन्द्रस्तमो हन्ति न च ताराः सहस्रशः ॥ 06 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              एक गुणवान पुत्र (या संतान) सैकड़ों निर्गुण संतानों से कहीं अधिक
              मूल्यवान होता है। जिस प्रकार अकेला चंद्रमा अंधकार को समाप्त कर
              देता है, वहीं हजारों तारे मिलकर भी वह प्रभाव नहीं उत्पन्न कर पाते।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> जैसे किसी कंपनी में एक काबिल कर्मचारी का योगदान
              कई औसत कर्मचारियों से ज्यादा महत्वपूर्ण हो सकता है, उसी प्रकार
              परिवार में एक गुणी संतान का महत्व सबसे अधिक होता है जो पूरे परिवार
              का नाम रोशन कर सकती है।
              <br />
              <br />
              <b>English:</b> For example, in a company, a highly skilled
              employee can be more valuable than many average workers.
              Similarly, in a family, one virtuous child can uplift the entire
              family’s reputation.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              संख्या की अपेक्षा गुण का महत्व अधिक होता है। चाहे संख्या कम हो,
              लेकिन गुणवत्ता के कारण उसका प्रभाव और योगदान अधिक हो सकता है।
            </p>
          </div>
        </div>

        {/* Verse 7 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            मूर्खश्चिरायुर्जातोऽपि तस्माज्जातमृतो वरः । <br />
            मृतः स चाल्पदुःखाय यावज्जीवं जडो दहेत् ॥ 07 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              मूर्ख व्यक्ति यदि लंबे समय तक जीवित भी रहता है, तो भी उसके स्थान
              पर जन्म लेते ही मर जाने वाला व्यक्ति बेहतर होता है। जन्म के तुरंत
              बाद मरा व्यक्ति केवल थोड़े दुःख का कारण बनता है, जबकि मूर्ख
              व्यक्ति अपने जीवनकाल के दौरान हमेशा दूसरों के लिए कष्ट का कारण
              बनता है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> जैसे किसी संस्थान में एक अयोग्य व्यक्ति प्रमुख
              पद पर बैठा हो, तो वह संस्थान के विकास में बाधा बन सकता है। उसके
              गलत निर्णय से संस्थान को नुकसान हो सकता है। इसके विपरीत, यदि वह
              व्यक्ति पद पर न होता या जल्दी ही इस्तीफा दे देता, तो संस्थान कम
              कष्ट से उभर सकता था।
              <br />
              <br />
              <b>English:</b> For example, if an unqualified person holds a
              significant position in an organization, they can hinder its
              progress with poor decisions. Their continuous presence can lead
              to ongoing issues. In contrast, if they had left the position
              early, the organization might have faced less trouble.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              मूर्खता का दीर्घकालिक प्रभाव केवल समस्या और दुख लाता है। जीवन में
              गुणवत्ता और बुद्धिमत्ता की उपस्थिति दूसरों के कल्याण के लिए अत्यंत
              महत्वपूर्ण है।
            </p>
          </div>
        </div>

        {/* Verse 8 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            कुग्रामवासः कुलहीनसेवा कुभोजनं क्रोधमुखी च भार्या । <br />
            पुत्रश्च मूर्खो विधवा च कन्या विनाग्निना षट्प्रदहन्ति कायम् ॥ 08 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              घटिया स्थान पर रहना, नीच कुल के लोगों की सेवा करना, खराब भोजन
              करना, क्रोधी स्वभाव की पत्नी, मूर्ख पुत्र और विधवा बेटी — ये छह
              बातें बिना अग्नि के ही व्यक्ति के शरीर और मन को जलाती रहती हैं।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> जैसे किसी व्यक्ति का काम ऐसे स्थान पर हो जहाँ
              रोजाना अशांति और गंदगी हो, साथ ही उसके परिवार में सदस्यों का
              स्वभाव भी झगड़ालू हो, और उसके बच्चों में से कोई पढ़ाई या अच्छी
              आदतें न सीखे, तो वह व्यक्ति जीवन में मानसिक और शारीरिक रूप से दुख
              और परेशानी में रहेगा।
              <br />
              <br />
              <b>English:</b> For instance, if a person lives in an environment
              filled with noise and pollution, has to work under ungrateful or
              disrespectful people, and has family members with constant anger
              issues and children lacking discipline, that person would
              experience mental and physical distress throughout their life.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              जीवन में स्थिरता, शांति और संतुलन बनाए रखने के लिए अनुकूल स्थान,
              अच्छे संगति, और शांतिपूर्ण पारिवारिक जीवन बहुत आवश्यक हैं। इनका
              अभाव व्यक्ति को मानसिक और शारीरिक रूप से कष्ट में डाल देता है।
            </p>
          </div>
        </div>

        {/* Verse 9 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            किं तया क्रियते धेन्वा या न दोग्ध्री न गर्भिणी । <br />
            कोऽर्थः पुत्रेण जातेन यो न विद्वान् न भक्तिमान् ॥ 09 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              जिस गाय से न तो दूध प्राप्त होता है और न ही जो गर्भवती होती है,
              उसका क्या लाभ है? उसी प्रकार, उस पुत्र का क्या लाभ है जो न तो
              विद्वान है और न ही श्रद्धावान।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> जैसे किसी घर में ऐसा बेटा हो जो पढ़ाई-लिखाई में
              कमजोर हो, न ही किसी प्रकार का गुण हो और न ही वह माता-पिता की सेवा
              में रुचि रखता हो, तो ऐसा बेटा केवल नाम का बेटा है, उसका कोई
              वास्तविक लाभ नहीं है।
              <br />
              <br />
              <b>English:</b> For example, if a family has a son who neither
              excels in studies nor has any virtues or dedication to family
              values, then he is a son only in name, providing no real benefit
              to the family.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              किसी भी वस्तु या व्यक्ति का महत्व तभी है जब वह गुण और उपयोगिता से
              भरा हो। माता-पिता के लिए ऐसा संतान ही सार्थक है जो विद्या, गुण और
              भक्ति में श्रेष्ठ हो।
            </p>
          </div>
        </div>

        {/* Verse 10 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            संसारतापदग्धानां त्रयो विश्रान्तिहेतवः । <br />
            अपत्यं च कलत्रं च सतां सङ्गतिरेव च ॥ 10 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              इस संसार के दुखों से पीड़ित लोगों के लिए तीन प्रकार के सुख या
              विश्राम के कारण होते हैं: संतान, जीवनसाथी और सज्जनों की संगति।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> जैसे किसी व्यक्ति को आर्थिक कठिनाई का सामना करना
              पड़ता है, तो उसके बच्चे उसके लिए प्रोत्साहन का स्रोत बनते हैं।
              उसका साथी उसके साथ कंधे से कंधा मिलाकर खड़ा होता है, और सज्जनों के
              संपर्क में आकर उसे जीवन में सही दिशा मिलती है।
              <br />
              <br />
              <b>English:</b> For instance, when a person faces financial
              hardship, their children serve as a source of encouragement. Their
              spouse stands by them, providing support, and the company of good
              people offers guidance and peace, helping them cope with life's
              challenges.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              जीवन के कठिन समय में संतान, सच्चा साथी, और सज्जनों की संगति ही
              सच्ची शांति और बल प्रदान करते हैं। ये तीनों ही जीवन को स्थिरता और
              संबल देने के मुख्य साधन हैं।
            </p>
          </div>
        </div>
        {/* Verse 11 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            सकृज्जल्पन्ति राजानः सकृज्जल्पन्ति पण्डिताः । <br />
            सकृत्कन्याः प्रदीयन्ते त्रीण्येतानि सकृत्सकृत् ॥ 11 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              राजा, विद्वान और कन्याएं — ये तीनों ही एक बार बोलते हैं या एक बार
              निर्णय लेते हैं। राजाओं का आदेश, विद्वानों का ज्ञान, और कन्या का
              विवाह — ये सभी बातें एक बार में ही ठहराई जाती हैं और इन्हें
              दोहराया नहीं जाता।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> जैसे किसी अदालत में जज द्वारा सुनाया गया फैसला
              अंतिम होता है और वह अपील न होने पर स्थायी हो जाता है, वैसे ही राजा
              के आदेश भी अंतिम होते हैं। उसी प्रकार एक विद्वान किसी विषय पर अपनी
              बात कहे, तो वह सुनिश्चित होता है कि वही अंतिम सत्य है। इसी तरह,
              विवाह संस्कार भी परंपरागत रूप से एक बार ही होता है।
              <br />
              <br />
              <b>English:</b> For instance, in a courtroom, a judge’s ruling is
              final unless appealed; similarly, a king’s decree is usually
              definitive. Likewise, when a scholar shares knowledge, they
              consider it the final word on the matter. And in traditional
              societies, a marriage is typically performed just once.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              कुछ महत्वपूर्ण निर्णय, जैसे राजा का आदेश, विद्वान का कथन और कन्या
              का विवाह, परंपरागत रूप से एक बार ही होते हैं और स्थायी माने जाते
              हैं। इस श्लोक का सार यही है कि जीवन के कुछ मामलों में एक ही बार का
              निर्णय बहुत मायने रखता है।
            </p>
          </div>
        </div>

        {/* Verse 12 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            एकाकिना तपो द्वाभ्यां पठनं गायनं त्रिभिः । <br />
            चतुर्भिर्गमनं क्षेत्रं पञ्चभिर्बहुभी रणः ॥ 12 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              तपस्या अकेले की जाती है, अध्ययन दो लोगों के साथ, गायन तीन लोगों के
              साथ, तीर्थयात्रा चार लोगों के साथ और युद्ध में अधिक लोगों के साथ
              होना चाहिए।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> एक व्यक्ति ध्यान के लिए एकांत में जाता है।
              अध्ययन के लिए, गुरु और शिष्य एक दूसरे से विचार साझा करते हैं।
              संगीत प्रस्तुति में एक गायक के साथ अन्य सहयोगी होते हैं।
              तीर्थयात्रा में चार लोग यात्रा करते हैं ताकि एक-दूसरे की सहायता कर
              सकें। और युद्ध में जितने अधिक योद्धा होते हैं, उतना ही उनका बल
              बढ़ता है।
              <br />
              <br />
              <b>English:</b> A person meditates alone for better focus. For
              study, a teacher and student exchange ideas. In a musical
              performance, a singer is often joined by accompaniments. In a
              pilgrimage, four people journey together for mutual support. And
              in battle, a large army enhances the chances of victory.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              श्लोक यह सिखाता है कि अलग-अलग कार्यों के लिए अलग-अलग समूहों की
              आवश्यकता होती है। सही उद्देश्य के अनुसार लोगों का समूह होने से
              कार्य का प्रभाव और सफलता बढ़ती है।
            </p>
          </div>
        </div>

        {/* Verse 13 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            सा भार्या या शुचिर्दक्षा सा भार्या या पतिव्रता । <br />
            सा भार्या या पतिप्रीता सा भार्या सत्यवादिनी ॥ 13 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              सही पत्नी वही होती है जो पवित्र, कुशल, अपने पति के प्रति समर्पित,
              उसे प्रसन्न रखने वाली और सच्चाई बोलने वाली हो।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> रमा अपने परिवार का बहुत ध्यान रखती है। वह अपने
              पति के प्रति समर्पित है, सच बोलती है, और हर काम को कुशलता से करती
              है। वह अपने पति को प्रसन्न रखने के लिए छोटे-छोटे प्रयास करती है,
              जो उसे एक आदर्श पत्नी बनाता है।
              <br />
              <br />
              <b>English:</b> Rama takes great care of her family. She is
              devoted to her husband, speaks the truth, and performs all her
              duties skillfully. She makes small efforts to keep her husband
              happy, which makes her an ideal wife.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              यह श्लोक सिखाता है कि एक आदर्श पत्नी की विशेषताएं क्या होनी चाहिए।
              यह संदेश देता है कि रिश्तों में सच्चाई, कुशलता, समर्पण और प्रेम से
              परिवार में सुख और संतोष आता है।
            </p>
          </div>
        </div>

        {/* Verse 14 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            अपुत्रस्य गृहं शून्यं दिशः शून्यास्त्वबान्धवाः । <br />
            मूर्खस्य हृदयं शून्यं सर्वशून्या दरिद्रता ॥ 14 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              जिस व्यक्ति के संतान नहीं है, उसका घर सूना रहता है। जिसके मित्र या
              रिश्तेदार नहीं हैं, उसके लिए दिशाएँ भी सूनी लगती हैं। मूर्ख
              व्यक्ति का हृदय खाली होता है, और दरिद्रता के कारण सब कुछ शून्य और
              व्यर्थ हो जाता है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> जैसे एक ऐसा व्यक्ति जिसकी संतान न हो, उसका घर
              खाली लगता है, उसे ऐसा प्रतीत होता है जैसे उसकी विरासत अधूरी रह गई
              है। बिना मित्रों और परिवार के, जीवन दिशाहीन और अकेलेपन से भरा होता
              है। इसी प्रकार, ज्ञान से रहित मूर्ख व्यक्ति का हृदय खाली होता
              है—वह जीवन की सच्चाइयों और समझ से दूर होता है। अंत में, दरिद्रता
              (गरीबी) तो व्यक्ति के जीवन के हर पहलू को खालीपन से भर देती है,
              क्योंकि अभाव में संतोष और स्थिरता नहीं मिल पाती।
              <br />
              <br />
              <b>English:</b> Consider a person without children; their home may
              feel empty, as though their legacy remains unfulfilled. Similarly,
              without friends and family, life can feel directionless and full
              of loneliness. A person without knowledge experiences a similar
              emptiness in their heart, as they lack understanding and awareness
              of life’s truths. Lastly, poverty drains every part of life,
              filling it with a profound sense of lack, as one cannot find
              contentment or stability in a life full of unfulfilled needs.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              यह श्लोक बताता है कि जीवन में संतान, मित्र-परिवार, ज्ञान और
              समृद्धि कितने महत्वपूर्ण हैं। इनके बिना जीवन में एक प्रकार की
              खालीपन और अर्थहीनता आ जाती है। ये चार तत्व—संतान, बंधु-बांधव,
              ज्ञान, और संपन्नता—व्यक्ति को जीवन में पूर्णता और संतोष का अनुभव
              कराते हैं। यदि इनमें से कोई भी अभाव हो, तो वह व्यक्ति के जीवन को
              शून्य और अधूरा बना सकता है।
            </p>
          </div>
        </div>

        {/* Verse 15 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            अनभ्यासे विषं शास्त्रमजीर्णे भोजनं विषम्। <br />
            दरिद्रस्य विषं गोष्ठी वृद्धस्य तरुणी विषम्॥ 15॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              बिना अभ्यास के ज्ञान, गलत पाचन से भोजन, गरीब व्यक्ति के लिए समाज
              का वातावरण और वृद्ध व्यक्ति के लिए युवा स्त्री का आकर्षण, ये सभी
              परिस्थितियों के अनुसार विष के समान हो सकते हैं।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> यदि कोई व्यक्ति शास्त्रों का सही अभ्यास नहीं
              करता तो शास्त्र ज्ञान उसके लिए भ्रम और हानिकारक साबित हो सकता है।
              जैसे यदि किसी व्यक्ति को पाचन की समस्या हो तो भोजन उसका स्वास्थ्य
              खराब कर सकता है। इसी तरह, गरीबों के लिए उनका समाज एक विष जैसा हो
              सकता है। वृद्ध व्यक्ति का युवा स्त्री के प्रति आकर्षण न केवल समाज
              के लिए अपितु उस व्यक्ति के लिए भी हानिकारक हो सकता है।
              <br />
              <br />
              <b>English:</b> If a person does not practice the knowledge they
              acquire, it becomes harmful, just like poison. Similarly, food
              becomes harmful if someone has indigestion. For a poor person,
              their environment and society can be like poison. An old man's
              attraction to a young woman can have negative consequences both
              for him and society.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              यह श्लोक यह सिखाता है कि किसी भी चीज का प्रभाव उसकी स्थिति और
              उपयोग पर निर्भर करता है। बिना उचित अभ्यास, पाचन या समाज के सही
              माहौल के, किसी भी चीज़ का प्रभाव हानिकारक हो सकता है।
            </p>
          </div>
        </div>

        {/* Verse 16 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            त्यजेद्धर्मं दयाहीनं विद्याहीनं गुरुं त्यजेत् । <br />{" "}
            त्यजेत्क्रोधमुखीं भार्यां निःस्नेहान्बान्धवांस्त्यजेत् ॥ 16 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              जो धर्म, दया, और ज्ञान से रहित हो, उसे गुरु के रूप में स्वीकार
              नहीं करना चाहिए। क्रोध और अहंकार से भरी हुई पत्नी और निःस्नेह
              रिश्तेदारों को भी त्यागना चाहिए, क्योंकि वे हमारे जीवन की शांति और
              समृद्धि को बिगाड़ सकते हैं।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> मान लीजिए कि आपके जीवन में एक व्यक्ति है जो
              हमेशा आपके निर्णयों पर सवाल उठाता है और नकारात्मकता फैलाता है। ऐसे
              व्यक्ति से दूर रहकर आप अपनी शांति बनाए रख सकते हैं और बेहतर जीवन
              जी सकते हैं।
              <br />
              <br />
              <b>English:</b> For example, if there is someone in your life who
              constantly questions your decisions and spreads negativity,
              distancing yourself from such a person can help maintain your
              peace and live a better life.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              हमें अपनी मानसिक और आत्मिक शांति बनाए रखने के लिए नकारात्मक और
              विषाक्त संबंधों से बचना चाहिए।
            </p>
          </div>
        </div>

        {/* Verse 17 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            अध्वा जरा देहवतां पर्वतानां जलं जरा । <br />
            अमैथुनं जरा स्त्रीणां वस्त्राणामातपो जरा ॥ 17 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              सतत भ्रमण करना व्यक्ति को बूढ़ा बना देता है. यदि घोड़े को हरदम
              बांध कर रखते है तो वह बूढा हो जाता है. यदि स्त्री उसके पति के साथ
              प्रणय नहीं करती हो तो बुढी हो जाती है. धुप में रखने से कपडे पुराने
              हो जाते है.
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> जैसे पानी में बुरी तरह जमे हुए कीड़े जल्दी मरे
              हुए दिखाई देते हैं, वैसे ही किसी व्यक्ति का एक स्थान पर स्थिर रहना
              और कोई कार्य न करना उसे बुढ़ापे की ओर ले जाता है।
              <br />
              <br />
              <b>English:</b> Just like bacteria stagnate and die in still
              water, a person who remains stagnant without any activity moves
              toward old age.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              यह श्लोक यह सिखाता है कि जीवन में निरंतरता और गतिशीलता बनाए रखना
              चाहिए। यदि हम अपने जीवन को स्थिर रखेंगे या शारीरिक और मानसिक
              क्रियाओं में कमी करेंगे, तो हम जल्दी बुढ़ापे का सामना कर सकते हैं।
            </p>
          </div>
        </div>

        {/* Verse 18 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            कः कालः कानि मित्राणि को देशः कौ व्ययागमौ । <br />
            कश्चाहं का च मे शक्तिरिति चिन्त्यं मुहुर्मुहुः ॥ 18 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              प्रगति करने वाले या विजय पाने वाले व्यक्ति को इन बातों पर रोज़ाना
              विचार करना चाहिए: समय कैसा चल रहा है मेरे मित्र कौन हैं मेरी
              आर्थिक स्थिति कैसी है देशों की स्थिति कैसी है मैं कौन हूं और मैं
              क्या हूं मेरी शक्ति किन बातों में है
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> व्यक्ति को अपनी दिनचर्या में समय, पैसे, और
              मित्रों के बारे में सोचने की आदत डालनी चाहिए, जैसे कि किसी काम में
              सफलता पाने के लिए सोच-समझ कर निर्णय लेना।
              <br />
              <br />
              <b>English:</b> A person should develop the habit of thinking
              about time, money, and friends in daily life, like making
              thoughtful decisions to succeed in a task.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              यह श्लोक यह सिखाता है कि जीवन में सही निर्णय लेने के लिए हमें समय,
              मित्र, संसाधन, और शक्ति पर लगातार विचार करना चाहिए।
            </p>
          </div>
        </div>

        {/* Verse 19 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            अग्निर्देवो द्विजातीनां मुनीनां हृदि दैवतम् । <br /> प्रतिमा
            स्वल्पबुद्धीनां सर्वत्र समदर्शिनः ॥ 19 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              ब्राह्मणों के लिए अग्नि भगवान हैं, मुनियों के लिए भगवान उनके हृदय
              में हैं, जो संकीर्ण सोच वाले हैं, उनके लिए भगवान की मूर्ति ही
              प्रतीक हैं, और जो लोग समदर्शी हैं, उनके लिए भगवान सर्वत्र हैं।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> एक व्यक्ति जो सिर्फ भौतिक दृष्टिकोण से देखता है,
              वह भगवान की मूर्ति को एक पत्थर समझता है। वहीं, एक साधु जिसे आत्मा
              की गहराई का ज्ञान है, वह उसी मूर्ति में भगवान का वास देखता है।
              <br />
              <br />
              <b>English:</b> A person who looks at things from a materialistic
              perspective sees the idol as just a stone. On the other hand, a
              sage who understands the depth of the soul sees the presence of
              God in that idol.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              यह श्लोक यह सिखाता है कि हमारी मानसिकता और दृष्टिकोण भगवान के
              प्रति हमारी आस्था और अनुभव को निर्धारित करते हैं। जो समदर्शी होते
              हैं, उनके लिए भगवान हर स्थान और हर स्थिति में होते हैं।
            </p>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Chapter4;
