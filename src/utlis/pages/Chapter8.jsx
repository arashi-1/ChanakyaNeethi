import React from "react";

const Chapter8 = () => {
  return (
    <div className="flex flex-col items-center">
      <div className="bg-orange-500 h-16 sm:h-20 md:h-24 lg:h-28 w-full flex items-center justify-center">
        <h1 className="font-samarkan text-3xl sm:text-4xl md:text-5xl lg:text-6xl text-black">
          Chanakya Neethi
        </h1>
      </div>

      <h1 className="font-samarkan text-5xl mt-12">Chapter 7</h1>

      <div className="relative flex flex-col items-center mt-10 m-5 max-w-screen-lg mx-auto">
        {/* Verse 1 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            अधमा धनमिच्छंति धनमानौ च मध्यमाः । <br />
            उत्तमा मानमिच्छंति मानो हि महतां धनम् ॥ 01 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              निम्न स्तर के लोग केवल धन की कामना करते हैं। मध्यम स्तर के लोग धन
              और मान दोनों की चाह रखते हैं। लेकिन श्रेष्ठ व्यक्ति केवल सम्मान की
              इच्छा रखते हैं, क्योंकि उनके लिए मान ही सच्चा धन है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> एक साधु केवल अपनी प्रतिष्ठा और सम्मान को महत्व
              देता है, जबकि व्यापारी धन और मान दोनों की चाह रखता है।
              <br />
              <br />
              <b>English:</b> A sage values only his honor and respect, whereas
              a businessman desires both wealth and recognition.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              यह श्लोक हमें यह सिखाता है कि सच्चे महान व्यक्तियों के लिए सबसे
              मूल्यवान चीज उनकी प्रतिष्ठा और आदर है। हमें भी भौतिक वस्तुओं से
              अधिक मान-सम्मान का महत्व समझना चाहिए।
            </p>
          </div>
        </div>

        {/* Verse 2 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            इक्षुरापः पयो मूलं तांबूलं फलमौषधम् । <br />
            भक्षयित्वापि कर्तव्याः स्नानदानादिकाः क्रियाः ॥ 02 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              गन्ना, पानी, दूध, कन्द, पान, फल और औषधि – इन वस्तुओं का सेवन करने
              के बाद भी स्नान, दान, उपासना आदि किये जा सकते हैं।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> यदि आपने फल और दूध का सेवन किया है, तो भी स्नान
              और पूजा का महत्त्व बना रहता है।
              <br />
              <br />
              <b>English:</b> Even after consuming fruits and milk, the
              importance of bathing and performing rituals remains.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              यह श्लोक हमें सिखाता है कि केवल भोजन या औषधियों से शरीर को शुद्ध
              करना पर्याप्त नहीं है। शारीरिक और मानसिक पवित्रता बनाए रखने के लिए
              धार्मिक और नैतिक कर्तव्यों का पालन करना भी उतना ही आवश्यक है।
            </p>
          </div>
        </div>

        {/* Verse 3 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            दीपो भक्षयते ध्वांतं कज्जलं च प्रसूयते । <br />
            यदन्नं भक्षयते नित्यं जायते तादृशी प्रजा ॥ 03 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              दीपक अंधकार को खाता है लेकिन कालिख उत्पन्न करता है। उसी प्रकार, जो
              जैसा अन्न खाता है, वैसा ही उसका परिणाम या संतति होती है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> जैसा भोजन आप करते हैं, वैसी ही आपकी सोच और शरीर
              पर उसका प्रभाव पड़ता है।
              <br />
              <br />
              <b>English:</b> The kind of food you eat influences your thoughts
              and physical well-being.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              यह श्लोक हमें सिखाता है कि हमें अपने खान-पान और कर्मों पर विशेष
              ध्यान देना चाहिए। हमारे जीवन के परिणाम और गुण उन्हीं पर आधारित
              होते हैं।
            </p>
          </div>
        </div>

        {/* Verse 4 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            वित्तं देहि गुणान्वितेषु मतिमन्नान्यत्र देहि क्वचित् प्राप्तं <br />
            वारिनिधेर्जलं घनमुखे माधुर्ययुक्तं सदा । <br />
            जीवान्स्थावरजंगमांश्च सकलान्संजीव्य भूमंडलं भूयः पश्य तदेव <br />
            कोटिगुणितं गच्छंतमंभोनिधिम् ॥ 04 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              धन को केवल गुणवान और बुद्धिमान लोगों को देना चाहिए, न कि कहीं भी
              या किसी को भी। मेघों के माध्यम से समुद्र का पानी मिठास प्राप्त कर
              पृथ्वी पर गिरता है, जिससे सभी जीव-जंतु और वनस्पतियां जीवन पाते
              हैं। अंततः वही जल, कई गुना बढ़कर, फिर से समुद्र में लौटता है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> यदि आप धन का उपयोग अच्छे कार्यों में करते हैं,
              तो वह समाज में कई गुना फलदायी होता है।
              <br />
              <br />
              <b>English:</b> If you utilize wealth for virtuous causes, it
              multiplies its value and benefits society.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              धन का सही उपयोग और वितरण इसे न केवल उत्पादक बनाता है, बल्कि इसका
              चक्र पुनः लौटकर कई गुना फल देता है। हमें यह ध्यान रखना चाहिए कि
              हमारे संसाधन हमेशा सही दिशा में लगाए जाएं।
            </p>
          </div>
        </div>

        {/* Verse 5 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            चांडालानां सहस्रैश्च सूरिभिस्तत्त्वदर्शिभिः । <br />
            एको हि यवनः प्रोक्तो न नीचो यवनात्परः ॥ 05 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              विद्वान् लोग जो तत्त्व को जानने वाले है उन्होंने कहा है की मास
              खाने वाले चांडालो से हजार गुना नीच यवन (विदेशी लोग) होते है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> जो लोग किसी अन्य देश में रहते हैं लेकिन वहां की
              संस्कृति, परंपराओं और नैतिक मूल्यों को नहीं अपनाते, वे उस देश के
              लिए निष्ठावान नहीं माने जा सकते।
              <br />
              <br />
              <b>English:</b> Those who live in a foreign land but fail to
              respect and adopt its culture, traditions, and moral values cannot
              truly be considered loyal to that land.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              इस श्लोक के अनुसार, हमें बाहरी व्यक्तियों या विदेशी लोगों पर भरोसा
              नहीं करना चाहिए, जो हमारी संस्कृति, परंपराओं और नैतिक मूल्यों को
              अपनाने में विफल रहते हैं। उनका व्यवहार हमारे समाज से भिन्न होता
              है, जिससे उनकी निष्ठा और विश्वास पर प्रश्न उठता है।
            </p>
          </div>
        </div>

        {/* Verse 6 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            तैलाभ्यंगे चिताधूमे मैथुने क्षौरकर्मणि । <br />
            तावद्भवति चांडालो यावत्स्नानं न चाचरेत् ॥ 06 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              शरीर पर मालिश करने के बाद, स्मशान में चिता का धुआ शरीर पर आने के
              बाद, सम्भोग करने के बाद, दाढ़ी बनाने के बाद जब तक आदमी नहा ना ले
              वह चांडाल रहता है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> जैसे कोई व्यक्ति जब तक स्नान नहीं करता तब तक
              शारीरिक रूप से अपवित्र रहता है, वैसे ही किसी भी कार्य के बाद
              शुद्धता बनाए रखना जरूरी है।
              <br />
              <br />
              <b>English:</b> Just like a person remains impure until they
              bathe, it is important to maintain purity after any action.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              इस श्लोक के अनुसार, शारीरिक शुद्धता बनाए रखना महत्वपूर्ण है। तेल
              मालिश, चिता का धुआं, सम्भोग या दाढ़ी बनाने के बाद, व्यक्ति को
              स्नान करना चाहिए, ताकि वह शारीरिक और मानसिक दृष्टि से शुद्ध और
              अपवित्रता से मुक्त हो सके।
            </p>
          </div>
        </div>

        {/* Verse 7 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            अजीर्णे भेषजं वारि जीर्णे वारि बलप्रदम् । <br />
            भोजने चामृतं वारि भोजनांते विषापहम् ॥ 07 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              अजीर्ण होने पर जल औषधि के समान होता है, और पचने के बाद जल बल
              प्रदान करने वाला होता है। भोजन करते समय जल अमृत के समान होता है,
              जबकि भोजन के अंत में जल विष का नाश करने वाला होता है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> अजीर्ण के बाद यदि हम गर्म पानी का सेवन करते हैं,
              तो वह पाचन में मदद करता है और शरीर को शुद्ध करता है।
              <br />
              <br />
              <b>English:</b> After indigestion, if we drink warm water, it
              helps in digestion and cleanses the body.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              यह श्लोक जल की महिमा को बताता है और यह स्पष्ट करता है कि जल का सही
              समय पर सेवन हमारे स्वास्थ्य के लिए अत्यंत लाभकारी है।
            </p>
          </div>
        </div>

        {/* Verse 8 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            हतं ज्ञानं क्रियाहीनं हतश्चाज्ञानतो नरः । <br />
            हतं निर्णायकं सैन्यं स्त्रियो नष्टा ह्यभर्तृकाः ॥ 08 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              जो व्यक्ति ज्ञानहीन और क्रियाहीन होता है, वह अज्ञान के कारण मरा
              हुआ होता है। इसी तरह, जो निर्णायक और सैन्य की शक्ति से वंचित होता
              है, वह भी पराजित होता है। महिलाएँ भी अपने पतियों से वंचित होने पर
              नष्ट हो जाती हैं।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> यदि किसी के पास सही जानकारी नहीं है और वह कुछ भी
              कार्य नहीं करता, तो वह अपनी जिंदगी में कभी सफलता नहीं पा सकता।
              <br />
              <br />
              <b>English:</b> If someone lacks proper knowledge and does
              nothing, they will never achieve success in life.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              यह श्लोक यह सिखाता है कि जीवन में ज्ञान, निर्णय क्षमता, और सही
              दिशा का होना अत्यंत महत्वपूर्ण है। बिना इन गुणों के जीवन निरर्थक
              हो सकता है।
            </p>
          </div>
        </div>

        {/* Verse 9 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            वृद्धकाले मृता भार्या बंधुहस्तगतं धनम् । <br />
            भोजनं च पराधीनं तिस्रः पुंसां विडंबनाः ॥ 09 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              वृद्धावस्था में मरे हुए पत्नी, रिश्तेदारों के हाथ में चला गया धन,
              और दूसरों पर निर्भर भोजन ये तीन बातें पुरुषों के लिए विडंबनाओं का
              कारण बनती हैं।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> एक व्यक्ति जो वृद्धावस्था में अपनों से बाहर हो
              जाता है और दूसरों पर निर्भर होता है, उसकी स्थिति बहुत ही
              विडंबनापूर्ण होती है।
              <br />
              <br />
              <b>English:</b> A person who is left alone in old age and becomes
              dependent on others is in a very deplorable situation.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              यह श्लोक हमें यह समझाता है कि जीवन में स्वाधीनता और परिवार की
              महत्ता को समझना जरूरी है, ताकि वृद्धावस्था में अपमानजनक
              परिस्थितियों का सामना न करना पड़े।
            </p>
          </div>
        </div>

        {/* Verse 10 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            नाग्निहोत्रं विना वेदा न च दानं विना क्रिया । <br />न भावेन विना
            सिद्धिस्तस्माद्भावो हि कारणम् ॥ 10 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              ग्निहोत्र के विना वेद पढना व्यर्थ है, दान और दक्षिणा के विना
              यज्ञादि क्रियाएँ निष्फल होती हैं, श्रद्धा और भक्ति के विना किसी भी
              कार्य में सफलता प्राप्त नहीं होती । अतः भावना ही सब सिद्धियों का
              मूल कारण है ।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> किसी भी कर्म को केवल रूप या दिखावे के लिए न
              करें, क्योंकि भावना और आस्था ही उसे सफल बनाती हैं।
              <br />
              <br />
              <b>English:</b> Do not perform any act just for the sake of form
              or appearance, as it is the feeling and faith that make it
              successful.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              यह श्लोक हमें यह सिखाता है कि हर कार्य, चाहे वह धार्मिक हो या
              अन्य, केवल बाहरी रूप से नहीं, बल्कि सही भावना और आस्था से किया
              जाना चाहिए, ताकि वह सफल और सिद्ध हो सके।
            </p>
          </div>
        </div>
        {/* Verse 11 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            न देवो विद्यते काष्ठे न पाषाणे न मृण्मये । <br />न भावेन विना
            सिद्धिस्तस्माद्भावो हि कारणम् ॥ 11 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              देवता न तो लकड़ी में, न पत्थर में, और न ही मिट्टी में होते हैं।
              सच्ची भावना के बिना कोई सिद्धि प्राप्त नहीं हो सकती। इसलिए, भावना
              ही प्रत्येक कार्य के पीछे का प्रमुख कारण है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> कोई व्यक्ति अगर मूर्ति के सामने केवल दिखावे के
              लिए पूजा करता है, तो वह व्यर्थ है। पूजा सच्चे हृदय और निष्ठा से
              करनी चाहिए।
              <br />
              <br />
              <b>English:</b> If a person performs worship in front of an idol
              just for show, it is futile. Worship should be done with a sincere
              heart and devotion.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              इस श्लोक का मुख्य संदेश यह है कि किसी भी धार्मिक कार्य या पूजा में
              बाहरी साधनों का महत्व नहीं है, बल्कि भावना और निष्ठा ही सबसे
              महत्वपूर्ण हैं। भावना के बिना किसी कार्य का फल प्राप्त नहीं होता।
            </p>
          </div>
        </div>
        {/* Verse 12 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            काष्ठपाषाणधातूनां कृत्वा भावेन सेवनम् । <br />
            श्रद्धया च तथा सिद्धिस्तस्य विष्णुप्रसादतः ॥ 12 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              यदि कोई व्यक्ति लकड़ी, पत्थर या धातु से मूर्ति बनाकर, उसे भावना और
              श्रद्धा के साथ पूजता है, तो उसे साक्षात् भगवान विष्णु सिद्धि देते
              हैं।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> कोई व्यक्ति मिट्टी से बनी मूर्ति की सच्चे मन से
              पूजा करता है, तो उसकी श्रद्धा ही उसे फल देती है।
              <br />
              <br />
              <b>English:</b> If a person worships an idol made of clay with a
              sincere heart, their faith is what grants them success.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              इस श्लोक का मुख्य संदेश यह है कि ईश्वर की कृपा प्राप्त करने के लिए
              साधन की साधारणता या भव्यता मायने नहीं रखती। भावना और श्रद्धा ही वह
              माध्यम है, जिसके द्वारा विष्णु या किसी भी देवता की कृपा प्राप्त की
              जा सकती है।
            </p>
          </div>
        </div>
        {/* Verse 13 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            न देवो विद्यते काष्ठे न पाषाणे न मृन्मये । <br />
            भावे हि विद्यते देवस्तस्माद्भावो हि कारणम् ॥ 13 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              न तो देवता लकड़ी में हैं, न पत्थर में, न मिट्टी में। देवता वास्तव
              में भावना में बसते हैं। इसलिए, भावना ही देवता की उपस्थिति का मुख्य
              कारण है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> यदि कोई बिना भावना के पत्थर की मूर्ति की पूजा
              करता है, तो वह व्यर्थ है। लेकिन यदि वही पूजा सच्चे मन और भक्ति से
              की जाए, तो देवता उसमें विद्यमान हो जाते हैं।
              <br />
              <br />
              <b>English:</b> If someone worships a stone idol without devotion,
              it is futile. But if the same worship is done with true faith and
              devotion, the deity resides in it.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              इस श्लोक का सार यह है कि ईश्वर भौतिक वस्तुओं में नहीं, बल्कि सच्ची
              भक्ति और भावना में उपस्थित होते हैं। इसलिए पूजा में भावना का होना
              अनिवार्य है, क्योंकि भावना ही ईश्वर की कृपा और उपस्थिति का कारण
              बनती है।
            </p>
          </div>
        </div>
        {/* Verse 14 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            शांतितुल्यं तपो नास्ति न संतोषात्परं सुखम् । <br />
            अपत्यं च कलत्रं च सतां संगतिरेव च ॥ 14 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              शांति के समान कोई तपस्या नहीं है, और संतोष से बढ़कर कोई सुख नहीं
              है। उत्तम जीवन के लिए संतान, पत्नी और सज्जनों की संगति अनिवार्य
              मानी गई है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> यदि मनुष्य संतोषी है और उसके पास शांत जीवन है,
              तो वह सबसे सुखी है, भले ही उसके पास अधिक धन-संपत्ति न हो।
              <br />
              <br />
              <b>English:</b> If a person is content and has a peaceful life,
              they are the happiest, even without immense wealth.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              यह श्लोक जीवन में शांति, संतोष, परिवार और सज्जनों की संगति के
              महत्व को रेखांकित करता है। यह सिखाता है कि ये गुण और संबंध ही जीवन
              को पूर्ण और सुखमय बनाते हैं।
            </p>
          </div>
        </div>
        {/* Verse 15 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            गुणो भूषयते रूपं शीलं भूषयते कुलम् । <br />
            प्रासादशिखरस्थोऽपि काकः किं गरुडायते ॥ 15 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              गुण सौंदर्य को सजाते हैं, और शील (आचरण) कुल की शोभा बढ़ाता है। महल
              की चोटी पर बैठा हुआ कौआ भी गरुड़ नहीं बन सकता।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> कोई व्यक्ति चाहे उच्च पद पर हो, लेकिन यदि उसमें
              गुण और शील नहीं है, तो वह आदरणीय नहीं हो सकता।
              <br />
              <br />
              <b>English:</b> A person may hold a high position, but without
              virtues and good character, they cannot command respect.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              यह श्लोक गुण और शील (चरित्र) के महत्व को दर्शाता है। यह संदेश देता
              है कि बाहरी रूप या स्थिति की तुलना में आंतरिक गुण और आचरण अधिक
              महत्वपूर्ण हैं।
            </p>
          </div>
        </div>
        {/* Verse 16 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            निर्गुणस्य हतं रूपं दुःशीलस्य हतं कुलम् । <br />
            असिद्धस्य हता विद्या ह्यभोगेन हतं धनम् ॥ 16 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              गुणों के बिना सौंदर्य व्यर्थ है, खराब आचरण वाले व्यक्ति का कुल
              नष्ट हो जाता है। असिद्ध (अप्राप्त) ज्ञान व्यर्थ है, और जिसका उपभोग
              न हो, वह धन भी व्यर्थ है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> यदि धन को केवल इकट्ठा किया जाए और उसका उपयोग न
              हो, तो उसका होना व्यर्थ है।
              <br />
              <br />
              <b>English:</b> Wealth that is hoarded without being utilized is
              as good as wasted.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              यह श्लोक हमें सिखाता है कि गुण, अच्छा आचरण, उपयोगी ज्ञान, और धन का
              सही उपयोग जीवन में महत्वपूर्ण हैं। इन चीजों के बिना, बाहरी दिखावा
              और संपत्ति का कोई मूल्य नहीं है।
            </p>
          </div>
        </div>
        {/* Verse 17 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            शुद्धं भूमिगतं तोयं शुद्धा नारी पतिव्रता । <br />
            शुचिः क्षेमकरो राजा संतोषो ब्राह्मणः शुचिः ॥ 17 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              भूमिगत जल पवित्र होता है, पतिव्रता नारी शुद्ध होती है। जो राजा
              अपनी प्रजा का कल्याण करता है, वह पवित्र है। संतोष ही ब्राह्मण की
              पवित्रता है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> एक राजा जो अपनी प्रजा के हित में काम करता है,
              उसे आदर्श राजा माना जाता है।
              <br />
              <br />
              <b>English:</b> A king who works for the welfare of his people is
              considered an ideal king.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              इस श्लोक का उद्देश्य समाज के विभिन्न वर्गों के लिए आदर्श मानदंड
              स्थापित करना है। शुद्धता केवल बाहरी नहीं, बल्कि आंतरिक गुणों और
              व्यवहार में भी होनी चाहिए।
            </p>
          </div>
        </div>
        {/* Verse 18 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            असंतुष्टा द्विजा नष्टाः संतुष्टाश्च महीभृतः । <br />
            सलज्जा गणिका नष्टा निर्लज्जाश्च कुलांगना ॥ 18 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              असंतुष्ट ब्राह्मण या विद्वान अपना ज्ञान खो देते हैं। संतुष्ट राजा
              अपनी प्रजा का कल्याण कर सकते हैं। लज्जाशील वेश्या अपना कार्य नष्ट
              कर देती है, जबकि निर्लज्ज कुलीन स्त्री अपने कुल का पतन करती है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> असंतोष के कारण विद्वान अपने ज्ञान का अपमान कर
              बैठते हैं।
              <br />
              <br />
              <b>English:</b> A dissatisfied scholar often disrespects their own
              knowledge.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              श्लोक का संदेश है कि संतोष, सही व्यवहार और परिस्थितियों के अनुसार
              आचरण हर वर्ग के लिए आवश्यक है। असंतोष और अनुचित व्यवहार केवल विनाश
              का कारण बनते हैं।
            </p>
          </div>
        </div>
        {/* Verse 19 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            किं कुलेन विशालेन विद्याहीनेन देहिनाम् । <br />
            दुष्कुलं चापि विदुषो देवैरपि स पूज्यते ॥ 19 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              विद्या के बिना व्यक्ति का बड़ा और प्रतिष्ठित वंश किसी काम का नहीं
              है। वहीं, जो विद्वान होते हैं, भले ही वे निम्न कुल से हों, देवताओं
              द्वारा भी पूजनीय होते हैं।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> राजा का बेटा होने से ही कोई बड़ा नहीं बनता, उसके
              गुण उसे महान बनाते हैं।
              <br />
              <br />
              <b>English:</b> Being the son of a king does not make one great;
              virtues do.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              यह श्लोक ज्ञान की सर्वोच्चता को रेखांकित करता है। केवल वंश या
              परिवार की प्रतिष्ठा पर निर्भर रहना व्यर्थ है, क्योंकि व्यक्ति को
              महान और पूजनीय उसके गुण और विद्या ही बनाते हैं।
            </p>
          </div>
        </div>
        {/* Verse 20 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            विद्वान्प्रशस्यते लोके विद्वान् सर्वत्र पूज्यते । <br />
            विद्यया लभते सर्वं विद्या सर्वत्र पूज्यते ॥ 20 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              जो व्यक्ति ज्ञानी होता है, उसे इस संसार में सराहा और सम्मानित किया
              जाता है। विद्या के द्वारा वह सब कुछ प्राप्त कर लेता है, क्योंकि
              विद्या सर्वत्र पूजनीय है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> एक साधारण व्यक्ति भी अपनी विद्या के कारण महान बन
              सकता है।
              <br />
              <br />
              <b>English:</b> If a person attains true knowledge, they are
              respected everywhere in society.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              इस श्लोक से यह सिद्ध होता है कि ज्ञान ही सबसे महत्वपूर्ण है,
              क्योंकि यह व्यक्ति को न केवल संसार में प्रतिष्ठित करता है, बल्कि
              हर स्थान पर सम्मान दिलवाता है। विद्या के बिना कोई भी सफलता अधूरी
              है।
            </p>
          </div>
        </div>
        {/* Verse 21 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            मांसभक्ष्यैः सुरापानैर्मुखैश्चाक्षरवर्जितैः । <br />
            पशुभिः पुरुषाकारैर्भाराक्रांता हि मेदिनी ॥ 21 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              जो लोग मांसभक्षण, शराब पीने और निराकार शब्दों का उपयोग करते हैं,
              वे पशुओं के समान हैं। ये लोग अपने कार्यों से पृथ्वी को भारी कर
              देते हैं और उनका जीवन बिना किसी वास्तविक उद्देश्य के भर जाता है।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> हमें अपने जीवन में विवेक का पालन करना चाहिए और
              अपने शब्दों में भी ज्ञान और उत्तमता का समावेश करना चाहिए।
              <br />
              <br />
              <b>English:</b> We should follow discernment in our life and
              ensure that our words are also filled with wisdom and excellence.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              यह श्लोक यह सिखाता है कि हमारे आहार, व्यवहार, और शब्दों का हमारे
              जीवन और समाज पर प्रभाव पड़ता है। एक आदर्श जीवन के लिए हमें
              पवित्रता और विवेक का पालन करना चाहिए।
            </p>
          </div>
        </div>
        {/* Verse 22 */}
        <div className="relative flex flex-col items-center mt-10 m-5">
          <h1 className="text-3xl text-center">
            अन्नहीनो दहेद्राष्ट्रं मंत्रहीनश्च ऋत्विजः । <br /> यजमानं दानहीनो
            नास्ति यज्ञसमो रिपुः ॥ 22 ॥
          </h1>
          <div className="mt-7">
            <p className="font-semibold mt-5 text-center">
              इस श्लोक में यह कहा गया है कि बिना अन्न के राष्ट्र नष्ट हो जाता
              है, बिना मंत्र के यज्ञ कराने वाले ऋत्विज़ नष्ट हो जाते हैं, और
              बिना दान के यज्ञ करने वाला यजमान भी नष्ट हो जाता है। इसके साथ ही
              यह भी कहा गया है कि संसार में यज्ञ के समान कोई शत्रु नहीं है। इसका
              तात्पर्य है कि यज्ञ को छोड़ने से सब कुछ नष्ट हो सकता है और यज्ञ के
              महत्व को कम नहीं किया जा सकता।
            </p>
            <br />
            <p>
              <b>सरल उदाहरण:</b>
              <br />
              <b>हिंदी में:</b> जैसे बिना आहार के शरीर कमजोर हो जाता है, वैसे ही
              बिना यज्ञ के समाज और राष्ट्र का विकास नहीं हो सकता।
              <br />
              <br />
              <b>English:</b> Just like the body weakens without food, society
              and the nation cannot prosper without yajna.
            </p>
            <br />
            <p>
              <b>निष्कर्ष:</b>
              <br />
              इस श्लोक का निष्कर्ष यह है कि यज्ञ के बिना कोई भी कार्य या
              अस्तित्व स्थिर नहीं रह सकता। अन्न, मंत्र और दान के बिना यज्ञ अपने
              उद्देश्य को प्राप्त नहीं कर सकता, और इस प्रकार यह श्लोक यह स्पष्ट
              करता है कि यज्ञ के समान कोई भी शत्रु नहीं है, जो समग्र रूप से सबको
              नष्ट कर सके।
            </p>
          </div>
        </div>
      </div>
    </div>
  );
};

export default Chapter8;
